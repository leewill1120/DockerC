...
myApp.controller('containerCloneController',function($scope, toastr, dockerService){
	var networks = {};
	var volumes = {};
	var defaultIPAddressNum = 100;
	var currentIPAddressNum = defaultIPAddressNum;
	var cloneSrcContainerId = URL.parse(document.URL,true).query.cloneSrcId;
	var ignoreEnvs = {PATH:''};
	var ignoreLabels = {DOCKERC_OWNER:'', DOCKERC_FIXED_IP:''};
	var readyCnt = 4;
	...
	var timer = setInterval(function(){
		...
		if(readyCnt == 0){
			...
			dockerService.containerInspect(localStorage.dockerIP, localStorage.dockerPort, cloneSrcContainerId)
				.success(function(data, status, headers){
					$scope.Info = data;
			...
			.success(function(data, status, headers){
				if(201 == status){
					...
					if(autoStart){
						...
						$scope.createStatus = "正在克隆...完成!";
						$scope.OnProgressing = "";
					}
				}else{
					$scope.createStatus = "克隆失败";
					$scope.colorStatus = "danger";
					$scope.OnProgressing = "";
					$scope.text = data;
				}
